...
class SkfInfo(object):
    ...
    def __init__(self, name, paramPath):
        ...
        with open(os.path.join(paramPath, name + '.skf')) as skfFile:
            ...
            self.__skTable = self.__SkTable(skfFile, nGridPoints, keys)
    ...
    def GetSkInt(self, key, dist):
        return self.__skTable[key](dist)
    ...
    def __SkTable(self, skfFile, numLines, keyList):
        table = []
        for _ in range(numLines):
            table += [_Parse(skfFile.next())]
        table = map(list, zip(*table)) # transpose
        skTableRaw = {key: val for key, val in zip(keyList, table)}
        ctor = InterpolatedUnivariateSpline
        skTable = {}
        for key in keyList:
            skTable[key] = ctor(self.__distSpace, skTableRaw[key], k=3, ext=1)
        return skTable
...
def _LineDict(skfFile, keyList):
    return {key: val for key, val in zip(keyList, _Parse(skfFile.next()))}
